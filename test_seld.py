...
tf.app.flags.DEFINE_string("dumping_dir", "./threshold_search_dump/", "Point to directory to dump results of threshold search")
tf.app.flags.DEFINE_string("dcase_dir", "./dcase_output/", "Point to directory to store results for dcase submission")
...
dumping_path = os.path.abspath(os.path.join(out_path,FLAGS.dumping_dir))
dcase_path = os.path.abspath(os.path.join(out_path,FLAGS.dcase_dir))
if not os.path.isdir(os.path.abspath(dumping_path)): os.makedirs(os.path.abspath(dumping_path))
if not os.path.isdir(os.path.abspath(dcase_path)): os.makedirs(os.path.abspath(dcase_path))
...
if params['mode'] == 'dev':
    test_splits = [1]
    val_splits = [2]
    train_check_splits = [3]
    train_splits = [3, 4, 5, 6]

elif params['mode'] == 'eval':
    test_splits = [7, 8]
    val_splits = [1]
    train_check_splits = [2]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        net = SELDnet(params, data_gen_valid.get_in_shape(),
                      data_gen_valid.get_out_shape_sed(),
                      data_gen_valid.get_out_shape_doa())
        ...
        best_dir = os.path.join(checkpoint_path, "best_model_seld")
        saver.restore(sess, best_dir)
        print("Model loaded seld frame")
        ...
        def log_dump_file(filename, threshold, sed_metric, doa_metric, seld_metric, new_metric, new_seld_metric):
            with open(os.path.join(dumping_path, filename), "a") as text_file:
                text_file.write("{:0.2f} ".format(threshold))
                # dcase 2019
                text_file.write("{:0.4f} {:0.2f} ".format(doa_metric[0], doa_metric[1]*100))
                text_file.write("{:0.4f} {:0.2f} ".format(sed_metric[0], sed_metric[1]*100))
                text_file.write("{:0.4f} ".format(seld_metric))
        ...
        applied_LR = learning_schedule(0)

        '''INITIAL EVALUATION'''
        print("{} Start evaluation".format(datetime.now()))

        test_loss_sed, test_loss_doa, test_output_loss, \
        test_total_loss, test_score_sed, test_score_doa = \
            evaluate(gen=data_gen_test, LR=applied_LR)
        ...
        print('Dumping recording-wise results in: {}'.format(dcase_path))

        test_filelist = data_gen_test._filename_list
        # Number of frames for a 60 second audio with 100ms hop length = 600 frames
        max_frames_with_content = len(data_gen_test._gen_list[0]._y_sed)
        ...
        frames_per_file = len(data_gen_test._gen_list[0]._y_sed)

        assert(test_score_sed.shape[0] // frames_per_file == len(test_filelist))

        for file_cnt in range(test_score_sed.shape[0] // frames_per_file):
            output_file = os.path.join(dcase_path, test_filelist[file_cnt].replace('.npy', '.csv'))
            dc = file_cnt * frames_per_file
            output_dict = feat_cls.regression_label_format_to_output_format(
                test_score_sed[dc:dc + max_frames_with_content, :],
                test_doa_pred[dc:dc + max_frames_with_content, :]
            )
            output_dict = feat_cls.convert_output_format_cartesian_to_polar(output_dict)
            feat_cls.write_output_format_file_submission(output_file, output_dict)